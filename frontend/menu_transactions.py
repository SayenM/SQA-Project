...
class FrontEnd:
    ...
    def load_accounts(self, filename: str):
        ...
        # Format: NNNNN_AAAAAAAAAAAAAAAAAAAA_S_PPPPPPPP (spec format with spaces)
        # [0:5]=number [5]=space [6:26]=name [26]=space [27]=status [28]=space [29:37]=balance
        with open(filename, "r") as file:
            for line in file:
                line = line.rstrip("\n").ljust(37)
                acc_number = line[0:5]
                name = line[6:26].replace("_", " ").strip()
                if name == "END OF FILE" or acc_number == "00000":
                    break
                status = line[27]
            ...
            try:
                with open("../current_accounts.txt", "r") as f:
                    for line in f:
                        ...
                        raw_name = line[6:26]
                        full_name = raw_name.replace("_", " ").strip()
    ...
    def append_daily_transaction(self, code, name, acc_number, amount=0, misc=""):
        # Add transaction to buffer in spec format (40-char fixed-width).
        # Format: CC_AAAAAAAAAAAAAAAAAAAA_NNNNN_PPPPPPPP_MM
        # CC(2)+space(1)+A(20)+space(1)+N(5)+space(1)+P(8)+MM(2) = 40
        name_field = name.replace(" ", "_")[:20].ljust(20, "_")
        acc_field = str(acc_number).zfill(5)
        amount_field = f"{amount:08.2f}"
        misc_field = misc[:2].ljust(2)
        line = f"{code} {name_field} {acc_field} {amount_field}{misc_field}"
        self.session_transactions.append(line)
